...
do -- register egg writer
  ...
  local function render_formspec(pos, player, assigns)
    ...
    if yspec then
      return yspec.render_split_inv_panel(player, 3, 4, { bg = "default" }, function (loc, rect)
        if loc == "main_body" then
          return fspec.label(rect.x + 1, rect.y, "Egg") ..
  ...
  local tiles = {
    ["active"] = {
      ...
      "maidroid_tool_egg_writer_right.png^[transformFX",
      {
        ...
        name = "maidroid_tool_egg_writer_front_active.png^[transformFX",
      ...
      {
        backface_culling = false,
        name = "maidroid_tool_egg_writer_front_active.png",
  ...
  local function get_nearest_egg_entity(pos)
    ...
    local target_entity_name = mod:make_name("egg_entity")
    for _, object in ipairs(all_objects) do
      if object:get_luaentity() ~= nil and object:get_luaentity().name == target_entity_name then
  ...
  local function on_activate(pos)
    local egg_entity = get_nearest_egg_entity(pos)
    egg_entity.object:set_properties{
      textures = {
        mod:make_name("empty_egg")
      }
    }
  ...
  local function on_metadata_inventory_put_to_main(pos)
    ...
    local egg_entity = minetest.add_entity(
      center_position,
      mod:make_name("egg_entity")
    )
  ...
  mod._aux.register_writer(mod:make_name("egg_writer"), {
    ...
    empty_itemname                        = mod:make_name("empty_egg"),
...
do -- register a definition of an egg entity
  ...
  minetest.register_entity(mod:make_name("egg_entity"), {
    hp_max           = 1,
    visual           = "wielditem",
    ...
    current_egg      = mod:make_name("empty_egg"),
    center_position  = nil,
    is_moving        = false,
    angle            = 0,
  })
